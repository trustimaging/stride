...
class Acquisitions(ProblemBase):
    ...
    def load_artifacts(self, shot_ids: Optional[List[int]] = None) -> None:
        """
        ...
        Wavelets are downloaded eagerly on the head (they are small and must
        be available before the PDE runs on each worker).

        Call this instead of :meth:`load` when observed data has already been
        uploaded to the artifact store (e.g. after a forward pass run with
        ...
        if warehouse is None:
            raise RuntimeError(
                ...
                'mosaic.set_artifact_warehouse() before load_artifacts()')
        ...
        for shot_id in shot_ids:
            ...
            wavelet_data = warehouse.pull_remote(key_wav)
